...
// if the first model is unavailable to the configured key, the next is tried.
...
function normalizePriority(value: unknown): PruningStep['priority'] {
...
export class GeminiService {
  ...
  private async runWithFallback<T>(
    ...
    throw new Error(
      ...
      'Sjekk /api/ai/health eller Vercel-miljøvariablene for AI-nøkler.',
    );
  }
...
- x/y: koordinater 0–100 i bildet`;
    ...
    return sanitizeComprehensiveAnalysis(this.extractJson<ComprehensiveAnalysisResult>(text, {} as ComprehensiveAnalysisResult));
  }
...
- recommendedDate: en dato i YYYY-MM-DD format`;
    ...
    return sanitizePruningPlan(this.extractJson<PruningPlan>(text, {} as PruningPlan));
  }
